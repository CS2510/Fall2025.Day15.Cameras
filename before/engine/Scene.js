...
class Scene {
    ...
    draw(ctx) {
        ...
        for (const layer of Engine.layers) {
            const layerGameObjects = Engine.currentScene.gameObjects.filter(go => go.layer == layer && layer != "UI")

            for (const gameObject of layerGameObjects) {
                gameObject.draw(ctx)
            }
        }
        ...
        for (const gameObject of Engine.currentScene.gameObjects.filter(go=>go.layer == "UI")) {
            gameObject.draw(ctx)
        }
